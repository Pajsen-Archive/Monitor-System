...
const width = 160
const height = 140
const interval = 5000 // interval for checking in milliseconds
let x
let y
let line
const alerts = []
...
$(function setup() {
    $.get({
        url: 'http://localhost:3001/measurements.json',
        cache: false,
    ...
    $.get({
        ...
        complete: function(returnData) {
            ...
            if (result > threshold) {
                ...
                //console.log("admin", `${new Date().toLocaleString()}: ${name} - ${result + unit} (>${threshold + unit})`)
                new Notification("Alert", { body: `${new Date().toLocaleString()}: ${name} - ${result + unit} (>${threshold + unit})` })
